...
async function createUniqueSlug(baseSlug: string) {
  ...
  let slug = baseSlug || "archival-audio";
  let counter = 2;
...
export async function POST(
  ...
  const titleBase = audio.name.replace(/\.[^/.]+$/, "").trim() || "Archival audio";
  const title = `${parent.title} / ${titleBase}`;
  const slug = await createUniqueSlug(slugify(title));
  const extension = audio.name.split(".").pop()?.toLowerCase() || "file";
  const path = `${slug}/audio/${crypto.randomBytes(8).toString("hex")}.${extension}`;
  ...
  return Response.json({ title, slug, audioUrl: audioData.publicUrl });
}

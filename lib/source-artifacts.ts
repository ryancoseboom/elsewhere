...
function sourceToneLabel(type: SourceType) {
  const labels: Record<SourceType, string> = {
    article: "old page",
    review: "review clipping",
...
function isLowQualitySourceText(value: string) {
...
function sourceTextCandidates(source: SourceArtifact) {
  return [
    ...(source.extracted_fragments || []),
    source.short_excerpt,
    source.paraphrased_summary,
  ]
    .map((item) => item?.trim())
    .filter((item): item is string => Boolean(item))
    .filter((item) => !isLowQualitySourceText(item));
}
...
export async function getSourceInterferenceSnippets({
  ...
  return selected.flatMap((source) => {
    const fragment = weightedSample(
      sourceTextCandidates(source),
      () => source.intensity || 1,
      1
    )[0];
    ...
    return {
      ...
      tone: `${sourceToneLabel(source.source_type)} / ${
        source.publication_date || "undated"
      }`,
